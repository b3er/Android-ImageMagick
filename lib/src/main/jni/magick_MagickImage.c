    ...
    (JNIEnv *env, jobject self)
{
    ...
    dimensionClass = (*env)->FindClass(env, "org/imagemagick/awt/Dimension");
    if (dimensionClass == 0) {
	throwMagickException(env, "Unable to locate class org/imagemagick/awt/Dimension");
	return NULL;
    }
    consMethodID = (*env)->GetMethodID(env, dimensionClass,
				       "<init>", "(II)V");
    if (consMethodID == 0) {
	throwMagickException(env, "Unable to construct org/imagemagick/awt/Dimension");
	return NULL;
    }
